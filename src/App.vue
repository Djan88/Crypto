  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input @keyup="inputHelper" @keypress.enter="addTicker" v-model="ticker" type="text" name="wallet"
                id="wallet"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Например DOGE" />
            </div>
            <div v-if="ticker" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span @click="appendCoin(coinItem.Symbol)" v-for="coinItem in actualaizedCoinList" :key="coinItem.Id"
      ...
      <div>
        ...
        <button v-if="!lastPage" @click="page += 1"
        ...
        <span>Фильтр: </span>
        <input v-model="filterParam" type="text"
      ...
      <template v-if="this.tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div v-for="t in paginatedTickers" :key="t.name" @click="curTicker = t" :class="{
...
export default {
  ...
  computed: {
    ...
    paginatedTickers() {
      ...
      let to = this.page * 6;
      this.filtredTickers.length <= to ? this.lastPage = true : this.lastPage = false
      return this.filtredTickers.slice(from, to)
    },
    selectedTicker() {
      this.graph = []
    },
  ...
  methods: {
    ...
    removeTicker(n) {
      ...
      if (this.filtredTickers.length === 0 && this.page > 1) {
        this.page -= 1
      }
    },
    ...
    // Подскази при вводе криптовалют
    inputHelper() {
      let counter = 0;
