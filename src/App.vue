<!--Удалять наблюдение за обновлением состояние если удален счетчик-->
<!--документация watch-->
<!--Добавить Empty state при фильтрации если ничего нет-->
<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
      ...
      <div>
        ...
        <button v-if="!lastPage" @click="page+=1" class="my-4 mx-2 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">Вперед</button>
        <span>Фильтр: </span>
        <input
            v-model="filterParam"
            type="text"
            class="pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md" placeholder="Что искать?"/>
      </div>
      ...
      <div v-if="!this.filteredTickers.length">Нет добавленных счетчиков или нет результатов удовлетворяющих фильтру</div>
      <section v-if="curTicker" class="relative">
...
export default {
  ...
  computed: {
    filteredTickers(){
      let from = (this.page -1) * 6;
      let to = this.page * 6;
      let result = this.tickers
          .filter(elem => elem.name.toUpperCase().includes(this.filterParam.toUpperCase()))
      result.length <= to? this.lastPage = true : this.lastPage = false
  ...
  watch:{
    filterParam(){
      this.page = 1;
      window.history.pushState(null,'',`${window.location.pathname}?filter=${this.filterParam}&page=${this.page}`)
    },
    page(){
      window.history.pushState(null,'',`${window.location.pathname}?filter=${this.filterParam}&page=${this.page}`)
    }
